...
@ultroid_cmd(pattern="setdb( (.*)|$)", category="Tools", fullsudo=True)
async def _(ult):
...
@ultroid_cmd(pattern="deldb( (.*)|$)", category="Tools", fullsudo=True)
async def _(ult):
...
@ultroid_cmd(pattern="rendb( (.*)|$)", category="Tools", fullsudo=True)
async def _(ult):
    match = ult.pattern_match.group(1).strip()
    if not match:
